...
clf=['Random Forest']
mod_feature = st.sidebar.selectbox('Model for Training and Testing Dataset', clf)
...
weekly_data = load_data(50)
center_info_data = load_center_data(50)
meal_data = load_meal_data(50)

st.subheader("BarGraph")
st.write(weekly_data)
#Bar Chart
Gradaa = st.selectbox('Select Parameter:',('Amount', 'Time','V1','V2', 'V28','V27','Class'))

st.bar_chart(weekly_data[Gradaa])



#histogram

df = pd.DataFrame(weekly_data[:200], columns = [Gradaa])
df.hist()
plt.show()
st.pyplot()
...
chart_data = pd.DataFrame(weekly_data[:40], columns=[Gradaa])
